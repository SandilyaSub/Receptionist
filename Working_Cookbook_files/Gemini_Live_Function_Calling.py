...
# Function declarations for the yahoo trigger and end session
sandy_says_hi_function = {
    "name": "sandy_says_hi",
    "description": "Prints 'sandy says hi' when the user says 'yahoo' in conversation",
    "parameters": {
        "type": "object",
        "properties": {
            "trigger_word": {
                "type": "string",
                "description": "The word that triggered this function (should be 'yahoo')"
            }
        },
...
end_session_function = {
    ...
    "description": "Ends the conversation session gracefully when the customer agrees to disconnect",
    "parameters": {
        "type": "object",
        "properties": {
            "reason": {
                "type": "string",
                "description": "Reason for ending the session (e.g., 'order_complete', 'customer_requested')"
            },
            "final_message": {
                "type": "string",
                "description": "Final message to say before disconnecting"
            }
        },
...
tools = [{"function_declarations": [sandy_says_hi_function, end_session_function]}]
...
IMPORTANT: If the customer says the word "yahoo" at any point during the conversation, immediately call the sandy_says_hi function with the trigger_word parameter set to "yahoo". This is a special trigger word that activates a greeting function.

IMPORTANT: After you have gathered all the cake order details (flavor, size, egg/eggless, add-ons, shape, occasion, message, customizations) and provided the customer with the final price and pickup time, politely ask if they would like you to disconnect the call or if they prefer to hang up themselves. If they say "yes" or "go ahead" or indicate they want you to disconnect, immediately call the end_session_gracefully function with appropriate reason and final_message parameters. This will close the session gracefully.
...
class AudioLoop:
    def __init__(self, video_mode=DEFAULT_MODE):
        ...
        self.token_summary = {
            ...
            "input_tokens_sum": 0,
            "output_tokens_sum": 0,
            "response_breakdown_items": []  # Store individual breakdown items
        }
        ...
        while True:
            ...
            async for response in turn:
                ...
                if hasattr(response, 'tool_call') and response.tool_call:
                    print(f"\n🔧 FUNCTION CALL DETECTED: {response.tool_call}")
                    function_responses = []
                    ...
                    for fc in response.tool_call.function_calls:
                        ...
                        # Handle the sandy_says_hi function
                        if fc.name == "sandy_says_hi":
                            # This is our custom function - print the message!
                            print("\n🎉 YAHOO DETECTED! sandy says hi 🎉\n")
                            
                            # Create function response
                            function_response = types.FunctionResponse(
                                id=fc.id,
                                name=fc.name,
                                response={"result": "sandy says hi message printed successfully!"}
                            )
                            function_responses.append(function_response)
                        
                        # Handle the end_session_gracefully function
                        elif fc.name == "end_session_gracefully":
                            reason = fc.args.get('reason', 'unknown')
                            final_message = fc.args.get('final_message', 'Thank you for calling!')
                            
                            print(f"\n🔚 SESSION END REQUESTED!")
                            print(f"📝 Reason: {reason}")
                            print(f"💬 Final message: {final_message}")
                            print("🔄 Initiating graceful session closure...\n")
                            ...
                            function_response = types.FunctionResponse(
                                ...
                                name=fc.name,
                                response={
                                    "result": "Session will be closed gracefully",
                                    "status": "closing",
                            ...
                            # Set a flag to close the session after this response
                    ...
                    # Check if session end was requested
                    if self.session_end_requested:
                        print(f"\n🔚 Session end confirmed. Closing gracefully...")
                        # Give a moment for the final message to be processed
                        await asyncio.sleep(1)
                        return  # This will exit the receive_audio method and trigger session cleanup
    ...
    async def send_initial_greeting(self):
        ...
        greeting_message = "Welcome to happy endings, how may I help you today?"
        print(f"\n🎙️ Sending initial greeting: {greeting_message}")
        
        # Send the greeting as a text message to start the conversation
